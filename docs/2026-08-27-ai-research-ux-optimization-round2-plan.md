...
>   - **首份实证(2026-08-28,v4.6.0 longTerm 运行)**:W4 三校验全部按设计工作(组合模块 1 次重试通过,周期结论方向词/待观察信号/whatWouldChange 全合规,拒批均为既有资产归因纪律);但暴露既有六词边界表的拒批死循环(「原因待确认」语义合规却无边界词 → 预算耗尽),**v4.6.1 已修**(App 入库前补写缺失证据说明,回归测试用真实失败文本)。

> **⚠️ 2026-08-28 与 intelligence-v2 线合流后的功能状态(d51e150)**:对方线按产品决定移除了「今日研判」摘要卡(InvestmentTodaySummaryCard + ResearchReadingGuideSheet,含锚点基建外迁与守护断言)。经确认**接受移除**,因此本计划中以下条目随宿主消失、**待重新安置**(锚点/横幅/深链基建仍在,新宿主待定):
> - W1.2 空态能力清单 + 向导/示例入口(摘要卡空态)
> - W2.3「今天一句话」hero(摘要卡顶部;Core 派生 TodayVerdictDerivation 与测试仍在,只缺展示宿主)
> - W2.4 详细模式开关(摘要卡 trailing;AppStorageKey 与 portfolioVerificationSection 消费端仍在)
> - 第一轮的四链路行摘要与「怎么读」指南入口
> R2 其余成果(通知/进度叙事/结论卡/向导本体/错过横幅/角标/排队/回指/明确性契约)已随合并全部进入 main(合并树 775 测试全绿)。
