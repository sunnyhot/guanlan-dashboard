...
// MARK: - ordinal→cardinal 防火墙（七轮 P1 审查结论）
//
// 刻意**不提供** ordinal→cardinal 转换 policy：direction×strength 到
// ±1/±0.6/±0.2 一类的映射只是序数评分，没有可验证的量纲 / 测量语义，
// 却会作为 Decimal 进入 weightedSum——这会重新打开 D002 要关闭的黑箱
// 评分通道。criterion 需要数值时引用其底层可测量 cardinal：
// - Factor 引擎产出的 FactorMetric（trend / momentum / volatility...）
// - Macro / Fundamental CardinalObservation
// 只有 ordinal、无可测量来源的 LLM Signal 保持 narrative（Presentation
// 层消费），不进入 criterion 数学运算。
