...
final class UIExperienceRegressionTests: XCTestCase {
    ...
    func testPlatformHoldingsShowsAssetClassAndAssetTypeAllocationByUnits() throws {
        ...
        let monthlySectionStart = try XCTUnwrap(platform.range(of: "// MARK: - Collapsible Monthly Section"))
        let holdingsSection = String(platform[holdingsSectionStart.lowerBound..<monthlySectionStart.lowerBound])
        let monthlySection = String(platform[monthlySectionStart.lowerBound..<platform.endIndex])
        ...
        XCTAssertTrue(holdingsSection.contains("isHoldingDetailsExpanded ? \"收起持仓详情\" : \"展开持仓详情\""))
        XCTAssertTrue(holdingsSection.contains(".accessibilityLabel(\"持仓详情\")"))
        XCTAssertFalse(monthlySection.contains("PlatformHoldingsPieChart("))
