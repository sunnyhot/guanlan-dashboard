...
**当前发布版本**: v4.5.2（GitHub Release + `releases/macos/latest.json`）
...
| `InvestmentIntelligenceV2/Presentation/` | PRES-1：DecisionNarrator（只解释不重决）/ ResearchNarrator / ArtifactQueryService（UI 唯一读面）。产品展示层（2026-08-26 重构）：`InvestmentIntelligenceDashboardSnapshot`（双端共用 DTO，状态全 enum）+ `DashboardProjector.dashboardSnapshot`（聚合入口；盘中/决策报告按 target ID 可解析性过滤，旧自复制 Target 产物只留历史审计）+ `IntelligencePresentationFormatter`（稳定文案 + `IntelligenceUserFacingError` 七类错误映射与恢复动作）。2026-08-27 审计补读面：closeReview 段、SignalDigest（信号→证据引用）、ActionCandidate（决策胜者计划动作）、Discovery 候选 invalidationNote、`researchEvidence(evidenceIDs:)`（A6 证据明细，取最新未废弃 vintage） |
| `InvestmentIntelligenceV2/Decision/` | DEC-*：criterion 求值比较 / TargetRebalancePlanner（Δw 唯一来源，D001）/ ConstraintGate / PortfolioDecisionArtifact + DecisionValidator + Replayer。2026-08-27 审计补回（A2/A3）：`DecisionCaseModels.swift`（四类决策事项 + 生命周期状态机 + 六选一复盘，确定性 ID）、`DecisionCasePolicy.swift`（V1 默认阈值表）、`DecisionCaseEngine.swift`（纯函数评估 + merge 状态机：墓碑不复活/自动关闭/风险再现重开/复查到期标记/复盘流转） |
| `InvestmentIntelligenceV2/Persistence/`（user-intent） | 用户意图文件事实源（2026-08-26 产品重构 P0，不进 SQLite、删库重放不丢）：`StrategicAllocationTargetStore`（战略目标 append-only 事件 + current 指针原子推进；五类完备写入门禁 `validateCompleteCoverage`，同 ID 异内容/伪 ID fail-closed）与 `StrategicAssetClassAssignmentStore`（持仓资产分类事件；用户桶恒优先于系统识别）。Target/分类只从用户动作产生（D000），LLM/Signal 无构造通道。2026-08-27 增：`DecisionCaseStore`（user-intent/decision-cases/ 一案一文件，覆盖式全量写）与 `LegacyDecisionCaseImport`（V1 decision-cases.json + journals 手写复盘导入，复制语义幂等） |
| `Core/Clients/` | 数据源 + LLM 传输层（契约层与诊断日志在 WF-4 迁入归位） |

> 旧三条 AI 链路（`Core/TrendResearch/` + `Core/Trend/` + `Core/NextHourGuidance*`）与 V1 Slice 0-7（`Core/InvestmentIntelligence/`）已于 2026-08-26（WF-4+WF-5）全部删除；`FundLookThrough.swift` 刻意保留（双端穿透 UI 在用，切换 V2 计算器属后续 UI 迁移）。2026-08-27 按 `docs/2026-08-27-v1-to-v2-lost-features-audit.md` 在 V2 上补回收盘复盘/决策事项/证据明细/自动调度等（详见该文档「处置结果」节）；App 侧接线在 `Core/AppModel/DecisionCases.swift`、`MarketCloseReviewV2.swift`、`IntelligenceAutomationLoop.swift` + `Core/IntelligenceAutomation.swift`（调度设置与评估器）；阅读指南在 `docs/research-reading-guide.md`。
